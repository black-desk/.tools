...
extraterm_install_prompt_integration () {
    ...
    if [[ ! "$POWERLEVEL9K_LEFT_PROMPT_FIRST_SEGMENT_START_SYMBOL" =~ "$LC_EXTRATERM_COOKIE" ]] ; then
        prefix=`echo -n -e "%{\0033&${LC_EXTRATERM_COOKIE};3\0007%}%?%{\0000%}"`
        export POWERLEVEL9K_LEFT_PROMPT_FIRST_SEGMENT_START_SYMBOL="${prefix}${POWERLEVEL9K_LEFT_PROMPT_FIRST_SEGMENT_START_SYMBOL}"
    fi
}
extraterm_install_prompt_integration
